...
| **apkcombo** | `apkcombo.com` | 🟡 medium-low | direct | No anti-bot over ~40 requests: `/download/apk` is byte-identical with a Chrome UA, with curl's, and with none. **301s must be followed** (non-canonical slug). **Search pagination does not exist:** `?page=2` and `?page=3` return the same 20 results. Matches by substring, so "no results" is rare. No published hash. XAPKs exist and **the extension lies**: the R2 object is `….apks`, the `content-disposition` calls it `.xapk`, and inside is an XAPK with `manifest.json` `xapk_version` 2 whose base is `<packageName>.apk`, not `base.apk`. **The download link comes in two forms, and reading one of them is reading half the store.** `/r2?u=<percent-encoded url>` points at Cloudflare R2; `/d?u=<base64 url>` points at `download.pureapk.com`. Measured 18/09/2026 over 22 apps from the store's own feed: **11 serve `/d?`, 11 serve `/r2?`, none both.** The adapter read only the first until then, dropped every anchor on a page of the other kind, and answered `NotFound` to every Install on half the catalogue — the listing still showed an Install button, because the version list on the same page filled in behind it. The second form is not new: the page captured on 29/08/2026 as `download-no-variants.html.gz` already carried one, and the fixture was named after the misreading. Either way the query is decoded rather than the redirect followed, and the raw parameter is read **before** percent-decoding, because apkcombo's base64 is the standard alphabet and `URLDecoder` turns a `+` into a space. On the pureapk form the file name is in `_fn` (base64 of a percent-encoded name) and there is **no expiry** to read: the redirect is signed when it is followed. R2 signature valid 4 h — and **every anchor carries its own signature**, so the same file in the download page's two panels arrives as two different URLs: measured 03/09/2026 on Spotify, `X-Amz-Expires=14399` against `14400`, because the two panels are signed a moment apart. A signed URL is **not an identity**: deduplication goes on the `objectKey`, and doing it on the URL published two versions sharing one `VersionRef`. `versionCode` only in the info table, inside `span.blur`. The `/latest-updates/feed` entries all begin `[apk_updated]` (96 of 96 on 03/09/2026) and the parser strips it — but **apkcombo also publishes app names that genuinely start with a bracket** (`[Official] Atomy shop` and three more from the same publisher, read off the listing and out of search, neither of which passes through the strip): a bracket in a title is **not** a symptom of a broken strip, and believing it was cost a red canary on 01/09/2026. |
...
| **pdalife** | `pdalife.com` | 🔴 very high | user-assisted only | The only store publishing iOS, PSP and Android in the same list. Cloudflare in **passive CDN** mode: no challenge on reads. **Search is not `/suggest/`**: that endpoint exists, is JSON and robots-allowed — and returns **ten results for every query, including ones with no matches**. The HTML search is used instead: 20 per page, declared `data-max_page`. **A search with nothing to give answers 404** since 06/09/2026 — a query matching nothing and any page past the last, both with the complete results page in the body (`data-max_page="-1"` on the first). Only the code moved, not the markup: the adapter reads that body and calls it empty **only** when `ul.catalog-list` is in it, because a genuinely missing address — a moved scheme, an empty slug — 404s without it. Queries must be slugified first. packageName **exists** but only inside `.game-download__stores`: the page's first `play.google.com` link is an advert **17 times out of 17**. No hash, no versionCode — `data-version_id` is not a versionCode, it grows site-wide. **Never positional selectors** (advert slot order is randomised server-side). **The listing was redesigned on or before 16/09/2026**, and it moved exactly two things the app reads: the screenshots, from `.game-gallery` (fotorama, linking `m_img*.jpg` behind `th_` thumbnails) to `.game-screenshots-rail` (magnific-popup, linking the full-size `img*.jpg` — 37 KB against 13 KB on the sampled app, all three sizes still served); and the requirements, from `ul.game-download__list li` holding `OS version: Android 2.2+` to `dl.game-information-facts` holding `<dt>OS</dt><dd>Android 2.2+</dd>`. What kept the old name is the lightbox — `game-gallery-close`, `game-gallery-image` — which `.game-gallery` does not match, so the selector found nothing and every listing on the store came back with **no screenshots**, and every version with **no minimum SDK**. The canary said so for three nights; nothing in the app would have. The same redesign added `game-download-version` to the version accordion's class list, which the parser does not read but a test's string surgery did. reCAPTCHA v3 on hop 2, where **two of the three buttons are adverts and one is a real `.apk`**. The RSS feed has **5 entries out of 100 dated in the future**, up to 2029. |
