...
data class StoreListingEntity(
    ...
    /**
     * `true` when the store marked **this** listing as a rework, in its own markup.
     *
     * `NOT NULL DEFAULT 0`, and the default is declared **twice** — here and in migration 6 → 7 —
     * for the reason written above `content_kind`: a default present in one and absent from the
     * other is the mismatch Room reports when it opens the database, i.e. on the user's device.
     *
     * ### Rows that existed before this column are not back-filled, and that is the answer
     *
     * The question migration 5 → 6 taught to ask is "where is this value already written". Here it
     * is written **nowhere**: it comes from a parse this build is the first to perform, so there is
     * no address to recover it from and no earlier column holding it. A pre-existing row therefore
     * reads `0`, which on the five stores that redistribute reworks means `ModifiedBuild.POSSIBLE`
     * and not `ModifiedBuild.NONE` — the prudent of the two, and the same
     * thing the badge would say for a store that never marks a single listing.
     */
    @ColumnInfo(name = "declared_modified", defaultValue = "0")
...
data class AppVersionEntity(
    ...
    /**
     * What this build asks the system for, or `NULL` where nobody has read its manifest.
     *
     * **Nullable on purpose**, unlike every other list column here: an app requesting no permissions
     * is ordinary — especially on F-Droid — so an empty list has to be able to mean exactly that. A
     * `NOT NULL DEFAULT '[]'` would have put "asks for nothing" on every row of the eight stores
     * whose permissions are only knowable from the downloaded file, which is the most reassuring
     * sentence the screen can produce and would have been produced by ignorance.
     *
     * On F-Droid it arrives with the index, before anything is downloaded. On the other eight it is
     * written after the pre-install verification and before the session is committed, from the
     * archive itself — so a row is `NULL` until this app has actually held that file.
     */
    @ColumnInfo(name = "permissions") val permissions: List<UsesPermission>? = null,
...
/**
 * A search somebody ran, so it can be offered back to them.
 *
 * ### The query is the primary key, and that is what makes it a history rather than a log
 *
 * Searching for the same thing twice is one entry that moved to the top, not two lines. With an
 * autogenerated id the list would fill with the word somebody is currently refining — five rows of
 * `f`, `fi`, `fir`, `fire`, `firefox` — which is the opposite of recalling anything.
 *
 * The stored form is what was **typed**, not the normalised one: it goes back into the text field,
 * and handing back a lowercased, accent-stripped version of what somebody wrote would be the app
 * correcting them.
 *
 * ### Its own table and not the DataStore
 *
 * It is a list with an ordering and a ceiling, read back sorted and pruned by age — which is a
 * query. `settings.proto` holds preferences, and every field in it must have an entry on the
 * Settings screen: a repeated field of search terms would be a control asking to be edited.
 */
@Entity(tableName = "search_history")
data class SearchHistoryEntity(
    @PrimaryKey val query: String,
    /** Last searched. The list is newest first, and repeating a search moves it rather than adds. */
