 ...
 *
 * Local and not `FakeSettingsRepository`: that one lives in `:core:testing`, which depends on
 * `:core:data`, and cannot be imported from here without a cycle.
 *
 * It lives in a file of its own and no longer inside `InstallRepositoryTest`, because the readers
 * have become three: since `SearchRepositoryImpl` applies "Show NSFW content", the search needs the
 * settings too. Three copies nested in three test classes would have been three places to update at
 * the next field — and two of them would only be discovered by the compiler, which is still the best
 * way, but needlessly repeated.
 */
internal class LocalSettings(
    showNsfwContent: Boolean = false,
    ...
    keepApkAfterInstall: Boolean = false,
    allowPreviewChannels: Boolean = false,
) : SettingsRepository {
    override val appearance = MutableStateFlow(AppearanceSettings())
    override val updates = MutableStateFlow(UpdateSettings())
    override val versions = MutableStateFlow(VersionSettings(allowPreviewChannels))
    ...
    override val search = MutableStateFlow(SearchSettings(showNsfwContent, storeTimeout))
    override val notifications = MutableStateFlow(NotificationSettings())
    override val diagnostics = MutableStateFlow(DiagnosticsSettings(logRequests))
    override val network =
        MutableStateFlow(NetworkSettings(blockUserAssistedChallenge = blockUserAssistedChallenge))
    override val storage = MutableStateFlow(StorageSettings(keepApkAfterInstall = keepApkAfterInstall))

    /**
     * The only setter this double really implements.
     *
     * The others are `Unit` because no test looks at what happens when they are written; here instead
     * the thing to prove is precisely that switching it on changes the version offered, and changing
     * it requires the flow to re-emit.
     */
    ...
    override suspend fun setMeteredNetworkAllowed(allowed: Boolean) = Unit
